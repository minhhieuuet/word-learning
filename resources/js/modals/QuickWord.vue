<template>
<modal name="quickword" height="auto" width="400px" :scrollable="true" :click-to-close="true" @before-open="beforeOpen" @before-close="beforeClose">
    ...
    <div class="content">
        ...
        <a-row>
         ...
         <a-row style="margin-top: 20px;">
            Danh mục
            <multiselect v-model="selectedCategories" select-label=" Nhấn để chọn" deselect-label=" Nhấn để bỏ chọn" placeholder="Vui lòng chọn" label="title" track-by="id" :options="categories" :multiple="true" :taggable="true"></multiselect>
        </a-row>
...
export default {
    ...
    methods: {
        ...
        createNewWord() {
            ...
            this.isLoading = true;
            rf.getRequest('WordRequest').quickStore(params).then((res) => {
                this.isLoading = false;
                this.$emit('refresh');
                this.$emit('created');
                this.$modal.hide('quickword');
                this.$message.success('Thêm từ thành công');
            }).catch((err) => {
                this.isLoading = false;
            });
        },
