...
<div>
    ...
    <div v-if="STEP == 2">
      ...
      <div class="question-layout">
        			<div id="question-box">
				<span class="question-content">{{this.currentQuestion.content}}</span>
			</div>
			<div id="answer-box">
        <div class="answer-group-1">
          				<div id="answer-one" class="answer" @click="checkResult(0)">
                    <span class="big-text">A.</span><span>{{this.currentQuestion.options[0].content}}</span>
                  </div>
                  <div id="answer-two" class="answer" @click="checkResult(1)">
                    <span class="big-text">B.</span><span>{{this.currentQuestion.options[1].content}}</span>
                  </div>
        </div>
        <div class="answer-group-2">
          				<div id="answer-three" class="answer" @click="checkResult(2)">
					<span class="big-text">C.</span><span>{{this.currentQuestion.options[2].content}}</span>
				</div>
				<div id="answer-four" class="answer" @click="checkResult(3)">
					<span class="big-text">D.</span><span>{{this.currentQuestion.options[3].content}}</span>
				</div>
        </div>
			</div>
      </div>
...
export default {
  data() {
    return {
      ...
      questions: [
        {
          content: 'Who is the first dude walk on the moon ?',
          options: [
              {
                content: 'Neo Amstrong',
                correct: true
              },
               {
               ...
               {
                content: 'Romero Lukaku',
                correct: false
              },
          ]
        },
        {
          content: 'Who is the first dude go to the galaxy ?',
          options: [
              {
                content: 'mot',
                correct: true
               ...
               {
                content: 'Donald Nice Trump',
                correct: false
              },
               {
                content: 'David Beckham fd',
                correct: false
              },
               {
                content: 'Romero',
  ...
  methods: {
    startGame() {
      ...
      this.playSound('start-play')
      setTimeout(() => {
        this.stopSound('start-play');
      })
    ...
    checkResult(answerIndex) {
      ...
      if(this.currentQuestion.options[answerIndex].correct) {
        ...
        this.$swal({
          ...
          text: "Bạn có muốn tiếp tục cuộc chơi",
          icon: "success",
          buttons: true,
        }).then((willContinue) => {
          if (willContinue) {
...
#answer-one, #answer-three {
  margin-left: -150px;
}
#answer-two, #answer-four {
  margin-left: 163px;
}
.question-layout {
  height: 340px;
